...
.then(data => { 
    ...
    all_women_products = dataRow1.filter((product) => {
      return product.category==="women";
    })

   // unique_products = [...new Set(women_products)];
   
   row.innerHTML=''
   if (t ==='low_to_high')
   {
    all_women_products.sort((a, b) => a.price - b.price)
  }
   else if(t==='high_to_low')
   {
    all_women_products.sort((a, b) => b.price - a.price)
   }

   all_women_products.forEach((product, index) => {
...
let selectDropDown=(y)=>{
  ...
  if (t ==='low_to_high')
  {
    women_products.sort((a, b) => a.price - b.price)
    dataRow1.sort((a, b) => a.price - b.price)
    selectSubCategory(p)
}
else if(t ==='high_to_low')
{
  women_products.sort((a, b) => b.price - a.price)
...
//row4.innerHTML+=product_container
 ...
 // changeDoc()
 //displayArr = [...women_products]
 women_products = dataRow1.filter((product) => {
  return product.subcategory===p;
})


row.innerHTML=''
//console.log('women_products', women_products.sort((a, b) => a.price - b.price))
 // console.log('dropdown_filter',dropdown_filter)
 // console.log('p',p)
  
  women_products.forEach((product, index) => {
    product_container =`<div class="col-lg-3 col-md-6 mb-4">
    <!-- Card-->
    <div class="card rounded shadow-sm border-0">
      <div class="card-body p-4"><img src=${product.thumbnail} alt="" class="img-fluid d-block mx-auto mb-3" style="
...
var addToCartBtn = document.querySelector('#addToCartBtn');
 ...
 selectedProduct = women_products[a]
// selectedProduct = all_women_products[a]
 localStorage.setItem('selectedProduct',JSON.stringify(selectedProduct))
 //console.log(selectedProduct) 
  }

  let checkout_all = (b)=>{
   console.log('data',all_women_products[b])
    selectedProduct = all_women_products[b]
    localStorage.setItem('selectedProduct',JSON.stringify(selectedProduct))
